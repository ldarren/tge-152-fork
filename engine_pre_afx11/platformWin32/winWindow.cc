...
// Arcane-FX
//    Changes:
//        cursor -- improved handling of cursor when windowed.
//
//    Cursor no longer disappears over titlebar. Window does not jump suddenly when
//    focus changes. Cursor is no longer forced to the center when you don't
//    want it to.
//
...
// DARREN MOD
// Enhanced unicode input and better fps control
...
U32 layout = reinterpret_cast<U32>(GetKeyboardLayout(0)); // DARREN MOD: enchanced unicode input
static bool composing = ((layout & 0x09) == 0);
...
static bool cursorWasCentered = false;
static Point2I savedCursor(0,0);

static void centerCursor(RECT* r)
{
  cursorWasCentered = true;
  S32 cx = (r->right + r->left) >> 1;
  S32 cy = (r->bottom + r->top) >> 1;
  SetCursorPos(cx, cy);
}

static void storeCursor(RECT* r)
{
  if (!cursorWasCentered)
  {
    POINT p; GetCursorPos(&p);
    savedCursor.set(p.x - r->left, p.y - r->top);
  }
}

static void restoreCursor(RECT* r)
{
  if (cursorWasCentered)
    SetCursorPos(savedCursor.x + r->left, savedCursor.y + r->top);
  cursorWasCentered = false;
}
// AFX CODE BLOCK (cursor) >>
...
static LRESULT PASCAL WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
   switch ( message )
   {
      ...
      case WM_ACTIVATE:
         ...
         {
            // AFX CODE BLOCK (cursor) <<
            // Storing the cursor pos here keeps the window from suddenly jumping
            // when reactivating.
            RECT r; GetWindowRect(winState.appWindow, &r);
            storeCursor(&r);
            /* ORIGINAL CODE
            setMouseClipping();
...
//         Con::printf("IME start comp");
      ...
      default:
      {
         if (sgQueueEvents)
         {
         	...
         	sgWinMessages.push_back(msg); // DARREN: solved reverse unicode problem
			}
      }
   }
...
static void OurDispatchMessages()
{
   ...
   while (sgWinMessages.size())
   {
      ...
      if ( !mgr || !mgr->isMouseActive() )
      {
		 ...
		 case WM_INPUTLANGCHANGE:
			 {
				layout = reinterpret_cast<U32>(GetKeyboardLayout(0));
				composing = ((layout & 0x09) == 0);
				break;
			 }
         case WM_IME_COMPOSITION:
            {
...
//               Con::printf("OMG IME notify");
      ...
      if ( !composing && (!mgr || !mgr->isKeyboardActive()) ) // DARREN: if unicode input dun insert char directly
      {
         switch ( message )
         {
            case WM_KEYUP:
...
void Platform::process()
{
   ...
   else if(!Game->isJournalReading() /*&& gWindowCreated*/ ) //DARREN MOD: comment off so that dedicated server can sleep as well
      Sleep( 1 ); //( getForegroundSleepTime() ); // give others some process time if necessary...
...
// Modernization Kit
         if( pfds[bestMatch].cAlphaBits != pPFD->cAlphaBits )
         {
            // prefer perfect match
            if (pfds[i].cAlphaBits == pPFD->cAlphaBits)
            {
               bestMatch = i;
               continue;
            }
            else if( pfds[i].cAlphaBits > pfds[bestMatch].cAlphaBits)
            {
               bestMatch = i;
               continue;
            }
...
*/
   if (event.elapsedTime < sgTimeManagerProcessInterval)
   {
	   Sleep(sgTimeManagerProcessInterval - event.elapsedTime);
	   event.elapsedTime = gTimer.getElapsedMS();
   }
