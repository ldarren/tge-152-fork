...
Player::Player()
{
   ...
   mActionAnimation.locked = false;
   mActionAnimation.forceSet = false;
   mState = MoveState;
   mFalling = false;
   mContactTimer = 0;
   mJumpDelay = 0;
...
void Player::processTick(const Move* move)
{
   ...
   Move aiMove;
	//DARREN MOD: mouse click movement. to force client control object uses AIPlayer function, remove
   // !move condition (result jerky motion) or add a flag (!move || keyFalg), but this flag need to
   // set each time a key is pressed, which is hacky. still waiting for better solution to
   // use mouse and keyboard to control the player at the same time
   //workable but bad way: if ((!move || keyFlag) && isServerObject() && getAIMove(&aiMove))
...
void Player::advanceTime(F32 dt)
{
   ...
   if (mActionAnimation.thread && mActionAnimation.thread->isDisable()) return; // DARREN MOD: disable action thread, temp hack
   updateActionThread();
   updateAnimation(dt);
   updateSplash();
   updateFroth(dt);
...
{
   //AiPlayer run animation fix http://www.garagegames.com/index.php?sec=mg&mod=resource&page=view&qid=11218
   if (!getControllingClient() && isGhost())      return; 
...
void Player::setActionThread(U32 action,bool forward,bool hold,bool wait,bool fsp, bool forceSet)
{
   ...
   if (anim.sequence != -1)
   {
      ...
      mActionAnimation.forceSet        = forceSet;
...
void Player::updateActionThread()
{
	   ...
	   !mActionAnimation.holdAtEnd && (mActionAnimation.delayTicks -= !mMountPending) <= 0)
   {
      ...
      if (mActionAnimation.action == PlayerData::NullAnimation || (!mActionAnimation.waitForEnd || mActionAnimation.atEnd)) // DARREN MOD: double check, animationDone() might change something
		  pickActionAnimation();
   }
...
U32 Player::packUpdate(NetConnection *con, U32 mask, BitStream *stream)
{
   ...
   if (stream->writeFlag(mask & ActionMask &&
	  ...
	  stream->writeFlag(mActionAnimation.forceSet); // DARREN MOD
...
void Player::unpackUpdate(NetConnection *con, BitStream *stream)
{
   ...
   if (stream->readFlag()) {
      ...
      if (isProperlyAdded()) {
         setActionThread(action,true,hold,true,fsp,forceSet);
...
ConsoleMethod( Player, setActionThread, bool, 3, 7, "(string sequenceName, bool hold, bool fsp, bool wait, bool forceSet)")
{
   bool hold = (argc > 3)? dAtob(argv[3]): false;
   bool fsp  = (argc > 4)? dAtob(argv[4]): true;
   bool wait  = (argc > 5)? dAtob(argv[5]): true;
   bool forceSet  = (argc > 6)? dAtob(argv[6]): false;
   return object->setActionThread(argv[2],hold,wait,fsp,forceSet); // DARREN MOD: replaced orig: return object->setActionThread(argv[2],hold,true,fsp);
}
...
// static 
bool Player::sCorpsesHiddenFromRayCast = true;
...
U32 Player::afx_packUpdate(NetConnection* con, U32 mask, BitStream* stream, U32 retMask)
{
  ...
  if (stream->writeFlag(mActionAnimation.thread && mActionAnimation.thread->isDisable() != mActionAnimation.locked))
  {
	mActionAnimation.locked = mActionAnimation.thread->isDisable();
	stream->writeFlag(mActionAnimation.locked); //DARREN MOD: action thread disable
  }
...
void Player::afx_unpackUpdate(NetConnection* con, BitStream* stream)
{
  ...
  if (stream->readFlag())
  {
	  // DARREN MOD: action thread disable
	  if (stream->readFlag()) lockActionThread();
	  else unlockActionThread();
  }
}
...
U32 Player::unique_anim_tag_counter = 1;
#define BAD_ANIM_ID  999999999

void Player::restoreAnimation(U32 tag, bool is_death_anim)
{
...
U32 Player::playAnimation(const char* name, F32 pos, F32 rate, F32 trans, bool hold, bool wait, bool is_death_anim)
{
  return playAnimation(getAnimationID(name), pos, rate, trans, hold, wait);
}

U32 Player::playAnimation(U32 anim_id, F32 pos, F32 rate, F32 trans, bool hold, bool wait)
{
  if (anim_id == BAD_ANIM_ID)
  {
    Con::printf("Player::playAnimation() problem. BAD_ANIM_ID");
    return 0;
  }
  ...
  if (seq_id == -1)
  {
    Con::printf("Player::playAnimation() problem. BAD_SEQ_ID");
    return 0;
  }
...
F32 Player::getAnimationDuration(const char* name)
{
  return getAnimationDuration(getAnimationID(name));
}

F32 Player::getAnimationDuration(U32 anim_id)
{
  if (anim_id == BAD_ANIM_ID)
    return 0.0f;
  S32 seq_id = mDataBlock->actionList[anim_id].sequence;
...
void Player::lockActionThread()
{
	mShapeInstance->setTimeScale(mActionAnimation.thread, mActionAnimation.forward? 1: -1);
	mShapeInstance->setPos(mActionAnimation.thread,0);
	mShapeInstance->setTimeScale(mActionAnimation.thread,0);
	mActionAnimation.thread->disableThread(true);
}

void Player::unlockActionThread()
{
	mShapeInstance->setTimeScale(mActionAnimation.thread, mActionAnimation.forward? 1: -1);
	mActionAnimation.thread->disableThread(false);
}

ConsoleMethod(Player, lockActionThread, void, 2, 2, "void lockActionThread()")
{
	object->lockActionThread();
}

ConsoleMethod(Player, unlockActionThread, void, 2, 2, "void unlockActionThread()")
{
	object->unlockActionThread();
}
