...
GuiTextListCtrl::GuiTextListCtrl()
{
   ...
   // Header MOD start
   mHeaderBGColor.set(191,191,191,255);
   mHeaderFontColor.set(255,255,255,255);
   // Header MOD end
}
...
void GuiTextListCtrl::initPersistFields()
{
   ...
   // Header MOD start
   addField("HeaderBGColor",           TypeColorI, Offset(mHeaderBGColor, GuiTextListCtrl));
   addField("HeaderFontColor",         TypeColorI, Offset(mHeaderFontColor, GuiTextListCtrl));
   // Header MOD end
}
...
// Header MOD start
ConsoleMethod(GuiTextListCtrl, addHeader, void, 3, 3, "(string text)"              "Sets the column headers.")
{	
	object->addHeader(argv[2]);
}
// Header MOD end
...
void GuiTextListCtrl::onMouseDown(const GuiEvent &event)
{
...
// Header MOD start
void GuiTextListCtrl::onRenderColumnHeaders(Point2I offset, Point2I parentOffset, Point2I headerDim)
{	
	RectI cellR(offset.x + headerDim.x, parentOffset.y, mBounds.extent.x - headerDim.x, headerDim.y);    
	dglDrawRectFill(cellR, mHeaderBGColor);   
	dglSetBitmapModulation( mHeaderFontColor );   
	const char *text = mHeader.text;   
	for(U32 index = 0; index < mColumnOffsets.size(); index++)   
	{      
		const char *nextCol = dStrchr(text, '\t');      
		if(mColumnOffsets[index] >= 0)      
		{         
			dsize_t slen;         
			if(nextCol)     slen = nextCol - text;         
			else            slen = dStrlen(text);         
			Point2I pos(offset.x + 4 + mColumnOffsets[index], offset.y);         
			RectI saveClipRect;         
			bool clipped = false;         
			if(mClipColumnText && (index != (mColumnOffsets.size() - 1)))         
			...
			dglDrawTextN(mFont, pos, text, slen, mProfile->mFontColors);         
			if(clipped)	dglSetClipRect(saveClipRect);      
		}      
		if(!nextCol)         break;      
		text = nextCol+1;   
	}
...
void GuiTextListCtrl::addHeader(const char *text)
{   
	mHeader.text = dStrdup(text);   
	setHeaderDim(Point2I(0,  mFont->getHeight() + 2));
}
// Header MOD end