...
// Arcane-FX
//    Changes:
//           enhanced-tiebreaking -- tiebreaking value changed from bool to S8 to allow
//               for larger range of tiebreaking options. At the time of this change,
//               the stock bool tieBreaker value was not used by anything.
...
class SceneRenderImage
{
  public:
   ...
   SceneRenderImage()
      : sortType(Normal),
        isTranslucent(false),
        // AFX CODE BLOCK (enhanced-tiebreaking) <<
        tieBreaker(0),
        /* ORIGINAL CODE
        tieBreaker(false),
        */
   ...
   // AFX CODE BLOCK (enhanced-tiebreaking) <<
   S8       tieBreaker;          ///< If two objects have the same points, this is used to determine sorting order.
   /* ORIGINAL CODE
...
struct FogVolume;
class Forest; // TGEForest Pack Addon
...
class SceneState
{
   ...
   friend class Forest; // TGEForest Pak Addon
