...
public:
   enum PublicConstants {
	  ...
	  MaxSkinModifiers = 8            ///< Dynamic_Skin_Modifiers. Maximum allowed slots for skin modifiers
   };
...
private:
   ...
   struct Thread {
	  ...
	  F32  speed;		///< DARREN MOD, thread speed. 3 significant places
	  bool transit;		///< DARREN MOD, transition enabled?
	  F32  duration;	///< DARREN MOD, transition duration. 2 significant places
	  bool simulPlay;	///< DARREN MOD, both transition seq play together?
   };
...
protected:
   ...
   struct SkinMod2
   {
      StringHandle skinLocation;	// xxx.FACE.lmale.png
      StringHandle skinName;		// spiky.face.lmale.png
   ...
   struct MountedImage {
	  ...
	  SkinMod2 skinModifier; // Dynamic_Skin_Modifiers. extended for ShapeBaseImage
...
public:
   ...
   enum ShapeBaseMasks {
	  ...
	  //SkinMaskMod     = Parent::NextFreeMask << 8,		///< Dynamic_Skin_Modifiers. can actually use existing SkinMask, if you already have all your bits used
	  SoundMaskN      = Parent::NextFreeMask << 8,       ///< Extends + MaxSoundThreads bits. original: 8
   ...
   /// @name Dynamic_Skin_Modifiers
   /// @{
   bool addSkinModifier(S32 slot, const char *skinLocation, const char *skinName, const char *maskName, 
   ...
   bool removeSkinModifier(S32 slot); ///< removes custom modifier
   void clearSkinModifiers(); ///< clears all modifiers on this shapeBase object
   bool addImageSkinModifier(S32 slot, const char *skinLocation, const char *skinName, const char *maskName, 
      const ColorI& printColor1, const ColorI& printColor2, 
      const ColorI& printColor3, const ColorI& printColor4 ); ///< set/update skin modifier
   bool removeImageSkinModifier(S32 slot); ///< removes custom modifier
   /// @}
   ...
   /// @param    continuePlay   new sequence plays during transition?
   /// @param   reset   Reset the sequence
   bool setThreadTransitSequence(U32 slot, S32 seq, F32 duration, bool continuePlay = true, bool reset = true);

   /// disable a thread
   /// @param   slot   Mount slot ID
   /// @param   disbale   disable this thread?
   bool disableThread(U32 slot,bool disable);
   ...
   /// DARREN MOD: change thread speed
   /// @param   slot   Mount slot ID
   /// @param   speed  speed in floating point value
   void setThreadSpeed(U32 slot,F32 speed);
