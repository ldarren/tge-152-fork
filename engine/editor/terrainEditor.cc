...
void EllipseBrush::rebuild()
{
   ...
   for(U32 x = 0; x < mSize.x; x++)
   {
      for(U32 y = 0; y < mSize.y; y++)
      {
         ...
         // In it's original form, the combination of inline type conversions assigned
         // bad values to the brush (for some compilers) and this limited out-of-bounds
         // terrain editing operations.
         S32 tmpX = mGridPos.x + x - (S32)center.x;
         S32 tmpY = mGridPos.y + y - (S32)center.y;
         mTerrainEditor->getGridInfo(Point2I(tmpX, tmpY), info);
         /* ORIGINAL CODE
