...
//-------------------------------------------------------------------------------------
   ...
   struct SkinMod2
   {
      ...
      long matIndex; ///< associated material index
      char modTag[32]; ///< unique tag (+[skinHash+maskHash+colors]\0
      GBitmap *modBmp; ///< the modifier bitmap that we apply to the skin

	  SkinMod2() : skinPath(0), matIndex(-1), modBmp(0) {};
      ~SkinMod2() {if (modBmp) delete modBmp;};
	  void clear() {skinPath = 0; matIndex = -1; modBmp = 0;};
   };

   SkinMod2 mSkinModifiers2[MaxSkinModifiers];
   void clearSkinModifiers2()
   {
	   for (int i = 0; i < MaxSkinModifiers; ++i)
			mSkinModifiers2[i].clear();
   }
   ///@}
   ...
   void uploadSkinModifier(U32 slot, StringHandle& skinLocation, StringHandle& skinName, StringHandle& maskName, 
      ...
      const ColorI& c3, const ColorI& c4 ); ///< set/update skin modifier
   void removeSkinModifier(U32 slot); ///< clear unneeded skin modifier
   void clearSkinModifiers(); ///< clear everything
   long whichMaterial(const char *skinLoc);  ///< get the index of specified material
...
class TSThread
{
   ...
   bool disabled;					  ///< DARREN MOD: let lower priority threads play?
...
public:
	inline bool disableThread(bool disable) { mShapeInstance->setDirty(TSShapeInstance::ThreadDirty); return (disabled = disable);}
	inline bool isDisable() const { return disabled; }
