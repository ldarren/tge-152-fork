...
StringTableEntry ResManager::sZipExt = ".zip";
...
void ResManager::searchPath (const char *path)
{
   ...
   for (U32 i = 0; i < fileInfoVec.size (); i++)
   {
      ...
      if (extension && !dStricmp (extension, sZipExt)) //".zip")) // DARREN MOD
      {
...
Stream * ResManager::openStream (ResourceObject * obj)
{
   ...
   if (obj->flags & ResourceObject::VolumeBlock)
   {
            ...
            || obj->fileSize == 0)
      {
         ...
         strm->attachStream (attachTo);
         strm->setStreamOffset (attachTo->getPosition (), obj->fileSize);
         return strm;
      }
      else
      {
            ...
            ZipLocalFileHeader::Deflated)
         {
            ...
            zipStream->attachStream (attachTo);
