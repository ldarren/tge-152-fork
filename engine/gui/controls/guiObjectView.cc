...
ConsoleMethod(GuiObjectView, addSkinModifier, bool, 6, 11, "(string meshName, U32 slot, string skinLocation, string skinName, string maskName, ColorI color1 [,color2,3,4='']) Add a skin modifier.")
{
   ...
   return object->addSkinModifier(argv[2],dAtoi(argv[3]),argv[4],argv[5],argv[6],col1,col2,col3,col4);
}
	...
	else
	{
		...
		for(S32 i=0; i<33; i++)
		{
			if(dStrcmp(mMeshObjects.mMesh[i].name, parentName) == 0)
			{
				...
				if(pNode == -1)
				{
					Con::errorf("Error: Unable to find node: %s", nodeName);
					return;
				}
		...
		if (pNode == -1 || pIndex == -1)
		{
			Con::errorf("Error: Unable to find mesh: %s", parentName);
			return;
		}
		...
		for (S32 i=0; i<33; i++)
		{
			...
			{
				Con::errorf("Unloading object %s at %s", mMeshObjects.mMesh[i].name, nodeName);
		...
		if (index == -1)
		{
			Con::errorf("Error: Maximum mountable objects reached. Please unMount an object");
			return;
		}
...
bool GuiObjectView::addSkinModifier( const char *meshName, U32 slot, const char *loc, 
                              ...
                              const ColorI& c3, const ColorI& c4)
{
	for(S32 i=0; i<33; i++)
	{
		if(dStrcmp(mMeshObjects.mMesh[i].name, meshName) == 0)
		{
			TSShapeInstance* ts = mMeshObjects.mMesh[i].mesh;
