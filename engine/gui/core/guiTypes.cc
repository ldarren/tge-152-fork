...
GuiControlProfile::GuiControlProfile(void) :
   ...
   mSoundDialogPush(0), // DARREN MOD: may return funny value. if not init properly in script, which is most the case
   mSoundDialogPop(0)
{
   mRefCount = 0;
   ...
   if (def)
   {
      ...
      mSoundDialogPush = def->mSoundDialogPush; // DARREN MOD
      mSoundDialogPop = def->mSoundDialogPop;
...
void GuiControlProfile::initPersistFields()
{
   ...
   addField("soundDialogPush", TypeAudioProfilePtr,  Offset(mSoundDialogPush, GuiControlProfile)); // DARREN MOD
   addField("soundDialogPop", TypeAudioProfilePtr,  Offset(mSoundDialogPop, GuiControlProfile));
