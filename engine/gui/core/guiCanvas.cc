...
void GuiCanvas::pushDialogControl(GuiControl *gui, S32 layer)
{
   ...
   // DARREN MOD: add audio feedback
   if (gui->mProfile->mSoundDialogPush)
   {
      AUDIOHANDLE handle = alxCreateSource(gui->mProfile->mSoundDialogPush);
      alxPlay(handle);
   }
}
...
void GuiCanvas::popDialogControl(GuiControl *gui)
{
   ...
   // DARREN MOD: add audio feedback
   if (gui->mProfile->mSoundDialogPop)
   {
      AUDIOHANDLE handle = alxCreateSource(gui->mProfile->mSoundDialogPop);
      alxPlay(handle);
   }
}
