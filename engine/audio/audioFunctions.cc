...
- Added console function to stop all except streaming (music) sources.
...
// Begin MMO Kit
//-----------------------------------------------
ConsoleFunction(alxStopAllExceptStreaming, void, 1, 1, "()")
{
...
// DARREN MOD: Audio Visualization
ConsoleFunction(alxGetOutputFFT, const char *, 4, 4, "alGetOutPutFFT(channel, Band)")
{
	S32 band = dAtoi(argv[3]);
	if (band < 1 || band > 32) band = 32;

	ALbyte * pBuffer = new ALbyte[band];

	alxGetOutputFFT(band, pBuffer);

	char * ret = Con::getReturnBuffer(512);
	dSprintf(ret, 512, "%g %g %g", 0, 0, 0);
