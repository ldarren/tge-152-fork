...
- Added function to stop all except streaming (music) sources.
- Added cleanup for streaming sources.
- Fixed device for win targets.
...
// Begin MMO Kit
void alxStopAllExceptStreaming()
{
   // stop all open sources, except streaming (music)
   S32 j;
   for(S32 i = mNumSources - 1; i >= 0; i--)
   {  
      if(mHandle[i] == NULL_AUDIOHANDLE)
      ...
      for (j = 0; j < mStreamingList.size(); j++)
         if (mHandle[i] == mStreamingList[j]->mHandle)
            break;
      
      if (j == mStreamingList.size())
         alxStop(mHandle[i]);
   }
...
void alxStreamingUpdate()
{
   ...
   for(StreamingList::iterator itr = mStreamingList.begin(); itr != mStreamingList.end(); itr++)
   {
      ...
      // Begin MMO Kit
      if(!(*itr)->updateBuffers())
      {
         cleanup.push_back(*itr);
      }
      /* TGE Original Code
      (*itr)->updateBuffers();
      */
   ...
   // Begin MMO Kit
   for(StreamingList::iterator itr = cleanup.begin(); itr != cleanup.end(); itr++)
      alxStop((*itr)->mHandle);
   // End MMO Kit
...
//   mDevice = (ALCdevice *)alcOpenDevice((ALCchar*)"DirectSound");
//#else
...
// DARREN MOD: audio visualization. need portAudio
ALbyte * alxGetOutputFFT(S32 band, ALbyte * buffer)
{
	if (!mDevice) return buffer;
#ifdef OPENAL11
	ALint samples = 0;

	alcGetIntegerv(mDevice, ALC_CAPTURE_SAMPLES, (ALCsizei)sizeof(ALint), &samples);
	alcCaptureSamples(mDevice, (ALCvoid *)mybuf, samples);
#endif
	return buffer;
}
