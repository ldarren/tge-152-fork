...
ConsoleMethod(afxEffectronData, addEffect, void, 3, 3, "(afxEffectWrapperData)") 
{
  afxEffectWrapperData* ew = dynamic_cast<afxEffectWrapperData*>(Sim::findObject(argv[2]));
  if (!ew) 
  {
    Con::errorf("afxEffectronData::addEffect() -- failed to find afxEffectWrapperData [%s].",
                argv[1]);
    return;
  }
  
  object->fx_list.push_back(ew);
}
...
afxEffectron::start_effect(afxEffectronData* datablock) 
{
  ...
  // CALL SCRIPT afxEffectronData::onActivate(%eff)
  Con::executef(datablock, 2, "onActivate", eff->scriptThis());
...
ConsoleFunction(startEffectron, S32, 2, 5, "startEffectron(datablock, [constraint_object], [constraint_name], [extra])")
{
  ...
  char* cons_obj_name_arg = 0;
  char* cons_name_arg = 0;
  SimObject* extra = 0;

  if (argc > 3)
  {
    cons_obj_name_arg = dStrdup(argv[2]);
    cons_name_arg = dStrdup(argv[3]);
  }
  ...
  // note -- we must examine all arguments prior to calling start_effect because
  // it calls Con::executef() which will overwrite the argument array.
  afxEffectron* eff = afxEffectron::start_effect(datablock);
  if (!eff)
  {
    if (cons_obj_name_arg) dFree(cons_obj_name_arg);
    if (cons_name_arg) dFree(cons_name_arg);
    return 0;
  }

  if (cons_obj_name_arg && cons_name_arg)
  {
    if (!eff->addConstraint(cons_obj_name_arg, cons_name_arg))
