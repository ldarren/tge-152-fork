...
void afxConstraintMgr::adjustProcessOrdering(afxChoreographer* ch)
{
   ...
   Vector<GameBase*> cons_sources;

   // add choreographer to the list
   cons_sources.push_back(ch);

   // collect all the GameBase related constraint sources
   for (S32 i = 0; i < constraints_v.size(); i++)
   {
      afxConstraintList* list = constraints_v[i];
      afxConstraint* cons = (*list)[0];
      if (cons)
      {
         GameBase* gobj = dynamic_cast<GameBase*>(cons->getObject());
         if (gobj)
            cons_sources.push_back(gobj);
      }
   }

   ProcessList* proc_list = (ch->isServerObject()) ? &gServerProcessList : &gClientProcessList;
   GameBase* nearest_to_end = proc_list->findNearestToEnd(cons_sources);

   // choreographer needs to be processed after the latest process object
   if (chor != nearest_to_end && nearest_to_end != 0)
   {
      //Con::printf("Choreographer processing BEFORE some of its constraints... fixing. [%s]",
      //   (ch->isServerObject()) ? "S" : "C");
      chor->processAfter(nearest_to_end);
   }
   else
   {
...
void afxTransformConstraint::set(MatrixF& xfm)
{
  this->xfm = xfm;
  is_defined = true;
  is_valid = true;
...
void afxTransformConstraint::sample(F32 dt, U32 elapsed_ms)
{
  last_xfm = xfm;
...
void afxTransformHistConstraint::set(MatrixF& xfm)
{
  if (!samples)
  {
    samples = new afxSampleXfmBuffer;
...
void afxTransformHistConstraint::sample(F32 dt, U32 elapsed_ms)
{
  Parent::sample(dt, elapsed_ms);
