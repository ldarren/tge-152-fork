...
private:
   ...
   SimObjectPtr<SimObject> mSelectedObj;  //pointer to our selected object   
   bool mChangedSelectedObj; //flag used to determine if we have changed our selected object

public:   
	///object selection additions   
	void setSelectedObject(SimObject *so) 
	{ 
		if (mSelectedObj) mSelectedObj->setHighLighted(false);
		mSelectedObj = so; 
		mChangedSelectedObj = true;
		mSelectedObj->setHighLighted(true);
	}   
	
	SimObject* getSelectedObject()  { return  mSelectedObj; }   
