...
void GameConnection::writePacket(BitStream *bstream, PacketNotify *note)
{
   ...
   else
   {
	  ...
	  {         
		  ...
		  {            
			  gIndex = getGhostIndex(reinterpret_cast<ShapeBase*>((SimObject*)mSelectedObj));            
			  if (bstream->writeFlag(gIndex != -1))               
				  bstream->writeInt(gIndex,20);         
		  }         
...
{   
	SimObject *sb = Sim::findObject(argv[2]);
	if(!sb) return false;   
