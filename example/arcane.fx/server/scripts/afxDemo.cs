...
$Total_Disabled_AFXModules = 0;
$Disabled_AFXModules[0] = "";

function autoloadAFXModules()
{
   if (!isFunction(afxGetEngine))
      $afxEngine = "TGE";
   else
      $afxEngine = afxGetEngine();

  %pattern = "~/server/scripts/effects/*/*.cs";
  %n_found = 0;
  %n_loaded = 0;
  for (%file = findFirstFile(%pattern); %file !$= ""; %file = findNextFile(%pattern)) 
  {
    ...
    // next check if script is disabled by filtering
    if ($pref::AFX::enableFXLoadFiltering)
    {
       for (%i = 0; %i < $Total_Disabled_AFXModules && %fx_is_enabled; %i++)
       {
          if (%file $= $Disabled_AFXModules[%i])
            %fx_is_enabled = false;
       }
    }
...
function clearAFXDisabledModules()
{
   $Total_Disabled_AFXModules = 0;
   $Disabled_AFXModules[0] = "";
}

function addAFXDisabledModules(%disabled_fx)
{
   $Disabled_AFXModules[$Total_Disabled_AFXModules] = %disabled_fx;
   $Total_Disabled_AFXModules++;
}
