...
{
   textureName          = "~/data/shapes/player/footpuff"; // AFX - added a real texture
   dragCoefficient      = 2.0;
...
datablock PlayerData(OrcWarriorAvatar) // AFX - was PlayerData(PlayerBody)
{
   renderFirstPerson = false;
   emap = true;
   
   className = Armor;
   shapeFile = "~/data/shapes/player/orc_warrior.dts"; // AFX - was player.dts
   cameraMaxDist = 3;
   computeCRC = false;    // AFX - was true
   ...
   density = 10;
   maxDamage = 100;
   maxEnergy =  0;        // AFX - was 60
   repairRate = 0.05;     // AFX - was 0.33
   energyPerDamagePoint = 75.0;

   rechargeRate = 0.0;    // AFX - was 0.256
...
// AFX CODE BLOCK <<
//  some different parameters for a mage.
datablock PlayerData(OrcMageAvatar : OrcWarriorAvatar)
{
   shapeFile = "~/data/shapes/player/orc_mage.dts";
   mass = 85;
   maxEnergy = 100;
   rechargeRate = 0.1;
};          
...
function Armor::damage(%this, %obj, %sourceObject, %position, %damage, %damageType)
{
   ...
   // AFX CODE BLOCK <<
   // This change allows negative damage to be used for healing.
...
function Armor::onDisabled(%this,%obj,%state)
{
   ...
   // AFX CODE BLOCK <<
   // we want the corpses to stick around in this demo so that
   // we can try out resurrection and corpse removal spells.
   /// ORIGINAL CODE
...
{
   // AFX CODE BLOCK <<
   // replace with a better sound clip...
...
// make sure players and bots kill any long-lasting
// mounted effects when they are removed

function OrcMageAvatar::onRemove(%this, %player)
{
...
function OrcWarriorAvatar::onRemove(%this, %player)
{
  if (isFunction(unequipSciFiOrc))
    unequipSciFiOrc(%player);
  if (isObject(%player.lingering_spell))
