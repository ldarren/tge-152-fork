function optionsDlg::setPane(%this, %pane)
{
   ...
   OptGamePane.setVisible(false);
   ("Opt" @ %pane @ "Pane").setVisible(true);
}

function OptionsDlg::onWake(%this)
{
   OptGameButton.performClick();
   ...
   // Game
	GuiGameDuration.setText($pref::Game::Duration);
	GuiGameURL.setText($pref::Game::HTTPHost);
	GuiGameFolderPath.setText($pref::Game::HTTPPath);
	GuiGameSortNumber.setText($pref::Game::HTTPSortNum);
	GuiGameID.setText($pref::Game::HTTPGameId);
	GuiStartQualify.setText($pref::Message::StartQualify);
	GuiEndQualify.setText($pref::Message::EndQualify);
}
...
function optionsDlg::applyGraphics( %this )
{
	...
	//$pref::Video::resolution = firstWord( %newRes ) SPC getWord( %newRes, 1 ) SPC %newBpp;
}

function optionsDlg::applyGame( %this )
{
	$timer = GuiGameDuration.getText();				//"2"
	$HTTPHost	=	GuiGameURL.getText();			//"www.mygozone.com:80";
	$HTTPPath	=	GuiGameFolderPath.getText();	//"/smsgame/moo/";
	$HTTPSortNum = GuiGameSortNumber.getText();	//"4007";
	$HTTPGameId =	GuiGameID.getText();				// "moo";
	
	$StartQualify = GuiStartQualify.getText();
	$EndQualify = GuiEndQualify.getText();
	
	$pref::Game::Duration			= $timer;
	$pref::Game::HTTPHost 			= $HTTPHost;
	$pref::Game::HTTPPath 			= $HTTPPath;
	$pref::Game::HTTPSortNum	 	= $HTTPSortNum;
	$pref::Game::HTTPGameId 		= $HTTPGameId;
	
	$pref::Message::StartQualify	=	$StartQualify;
	$pref::Message::EndQualify		=	$EndQualify;
	
	if(($StartQualify !$= "") && ($EndQualify !$= ""))
		$pref::Message::Msg8 = "<br>Qualifying period : <color:ffff00>"@$StartQualify@"<color:ffffff> to <color:ffff00>"@$EndQualify@"<color:ffffff>";
}
