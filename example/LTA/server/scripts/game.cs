...
function onMissionLoaded()
{
	...
	spawnNPC("LTA/data/dialogs/npc.xml");
	spawnItems("LTA/data/items/items.xml");
...
function GameConnection::onClientEnterGame(%this)
{
	...
	{      
		dataBlock = AdvOrbitCameraData;//Adv3rdCameraData;   
	};   
	...
	%argc = %this.getConnectArgsCount();
	%savedInfo = %this.getConnectArgs(1);
	//%count = getWordCount(%saveInfo);
	$checkpointtoload = getWord(%savedInfo, 0);
	%client.cash = getWord(%savedInfo, 1);
	if (%this.cash $= "") %this.cash = 0;
	%client.bank = getWord(%savedInfo, 2);
	if (%this.bank $= "") %this.bank = 0;
	%client.card = getWord(%savedInfo, 3);
	if (%this.card $= "") %this.card = 0;
	%client.fare = getWord(%savedInfo, 4);
...
//	%this.camera.setTransform(%player.getEyeTransform());
	...
	// set player skins
	%player.addSkinModifier(true, 0, "head", %headS);
	%player.addSkinModifier(true, 1, "body", %bodyS, "mask", "", "255, 0, 0, 255");
	%player.addSkinModifier(true, 2, "arms", %armsS);
	%player.addSkinModifier(true, 3, "hands", %handsS);
	%player.addSkinModifier(true, 4, "legs", %legsS);
	%player.addSkinModifier(true, 5, "feet", %feetS);
