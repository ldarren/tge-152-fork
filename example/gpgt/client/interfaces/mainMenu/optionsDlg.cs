...
function OptAudioUpdateMasterVolume(%volume)
{
   ...
   {
      $AudioTestHandle = alxCreateSource("AudioChannel0", expandFilename("~/data/GPGTBase/sound/testing.ogg"));
...
function OptAudioUpdateChannelVolume(%channel, %volume)
{
   ...
   if (!alxIsPlaying($AudioTestHandle))
   {
      $AudioTestHandle = alxCreateSource("AudioChannel"@%channel, expandFilename("~/data/GPGTBase/sound/testing.ogg"));
      alxPlay($AudioTestHandle);
   }
}
