...
// Arcane-FX
//    Changes:
//        enhanced-light-obj -- made code changes to allow linkage between a USL effect
//            and an emitter effect.
...
sgLightObjectData::sgLightObjectData()
{
  ...
  // AFX CODE BLOCK (enhanced-light-obj) <<
  afx_init();
  // AFX CODE BLOCK (enhanced-light-obj) >>
}
...
void sgLightObjectData::afx_initPersistFields()
{
  addField("emitterEffectName", TypeString, Offset(afx_emitterEffectName, sgLightObjectData));
}

void sgLightObjectData::afx_init()
{
  afx_emitterEffectName = StringTable->insert("");
}

void sgLightObjectData::afx_packData(BitStream* stream)
{
  stream->writeString(afx_emitterEffectName);
}

void sgLightObjectData::afx_unpackData(BitStream* stream)
{
  afx_emitterEffectName = stream->readSTString();
}

////////////

void sgLightObject::afx_setEffectEmitter(ParticleEmitter* emitter)
{
  sgParticleEmitterGhostIndex = 0;
  sgParticleEmitterPtr = emitter;
}
