...
#ifdef AFX_TGE_1_3
  Con::registerType("AfxMagicSpellData", TypeafxMagicSpellData, sizeof(afxMagicSpellData*),
                    REF_GETDATATYPE(afxMagicSpellData), REF_SETDATATYPE(afxMagicSpellData));
#endif
...
void afxMagicSpellData::gather_cons_defs(Vector<afxConstraintDef>& defs, afxEffectList& fx)
{
  for (int i = 0; i <  fx.size(); i++)
  {
...
void afxMagicSpell::init_constraints()
{
  ...
  else
  {
    ...
    listener_cons_id = constraint_mgr->setReferencePoint(LISTENER_CONS, listener_pos, Point3F(0,0,1));
  }
                                        ...
                                        const Point3F& pos, const char* data)
{
  ...
  // CALL SCRIPT afxMagicSpellData::method(%spell, %caster, %constraint, %pos, %data)
...
ConsoleFunction(castSpell, S32, 4, 5, "castSpell(spell_datablock, caster, target, [extra])")
{
  ...
  afxMagicSpell* spell;
  spell = afxMagicSpell::cast_spell(datablock, caster, target);
  if (!spell)
    return 0;
