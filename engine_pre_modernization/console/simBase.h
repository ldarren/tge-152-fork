...
// Arcane-FX
//    Changes:
//        datablock-id-size	-- DataBlockObjectIdBitSize changed from 10 to 12.
//        obj-select		-- object selection functionality **** Moved from sceneObject
...
enum SimObjectsConstants
{
   ...
   // This change adds two bits to the datablock-id size, DataBlockObjectIdBitSize, 
   // increasing the allowable datablock limit from 1023 to 4095. AFX tends to require
   // a large number of datablocks and the spells implemented in the AFX demo exceeded
   // the standard limit. Bumping this value to 11 or 12 is recommended for anyone
...
class SimObject: public ConsoleObject
{
  ...
  private:
   ...
   enum {
	  ...
	  Clicked = BIT(8),			///< This object has been picked by mouse cursor in game, mod for object selection additions
	  Hovered = BIT(9)			/// object selection additions
   };
  public:
   /// @name Notification
   /// @{
...
//     SELECTED marks an object as selected (a target)
   bool isClicked() const { return mFlags.test(Clicked); }
   void setClicked(bool down) { if(down) mFlags.set(Clicked); else mFlags.clear(Clicked); }
   bool isHovered() const { return mFlags.test(Hovered); }
   void setHovered(bool over) { if(over) mFlags.set(Hovered); else mFlags.clear(Hovered); }
// AFX CODE BLOCK (obj-select) >>
